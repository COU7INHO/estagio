...
st.set_page_config(
    ...
    menu_items={
        'About': 'Software para analise da marcha em amputados do membro inferior'
    }
)
...
def create_pdf(img1, img2, img3, file_name):
    ...
    pdf.set_xy(30, 77)
    pdf.set_font('times', 'B', 12)
    pdf.cell(w=30, txt='Membro amputado: ')
    pdf.set_x(69)
    pdf.set_font('times', '', 12)
    ...
    pdf.cell(w=47.6, txt='Referência do joelho', border=1, align='c')
    pdf.set_xy(30 + 47.6, 83)
    pdf.cell(w=47.6, txt='Referência do pé', border=1, align='c')
    pdf.set_xy(30 + 47.6 * 2, 83)
    pdf.cell(w=47.6, txt='Tipo de suspensão', border=1, align='c')

    pdf.set_xy(30, 86.5)
    pdf.set_font('times', '', 12)
    if amputation_level == 'Transtibial':
        pdf.cell(w=47.6, txt='N/A', border=1, align='c')
    if amputation_level == 'Transfemoral':
        pdf.cell(w=47.6, txt=knee_ref, border=1, align='c')
    pdf.set_xy(30 + 47.6, 86.5)
    pdf.cell(w=47.6, txt=foot_ref, border=1, align='c')
    pdf.set_xy(30 + 47.6 * 2, 86.5)
    pdf.cell(w=47.6, txt=susp_type, border=1, align='c')

    pdf.set_xy(30, 96)
    pdf.set_font('times', 'B', 14)
    pdf.cell(w=143, txt='Plano Sagital ', align='c')

    pdf.set_font('times', 'B', 12)
    pdf.set_xy(30, 102)
    pdf.cell(w=100, txt='Desvios verticais')

    pdf.set_font('times', '', 12)
    pdf.set_xy(30, 108)
    r_h, r_v, l_h, l_v, diff_h, diff_v = legs_comparasion(merged, merged_2, 'A_H', 'A_V')
    if r_v > l_v:
        pdf.cell(w=143, txt='O ombro direito subiu mais %.4s cm' % diff_v)
    else:
        pdf.cell(w=143, txt='O ombro esquerdo subiu mais %.4s cm' % diff_v)

    pdf.set_xy(30, 114)
    r_h, r_v, l_h, l_v, diff_h, diff_v = legs_comparasion(merged, merged_2, 'GT_H', 'GT_V')
    if r_v > l_v:
        pdf.cell(w=143, txt='O trocanter direito subiu mais %.4s cm' % diff_v)
    else:
        pdf.cell(w=143, txt='O trocanter esquerdo subiu mais %.4s cm' % diff_v)

    pdf.set_xy(30, 120)
    r_h, r_v, l_h, l_v, diff_h, diff_v = legs_comparasion(merged, merged_2, 'LE_H', 'LE_V')
    if r_v > l_v:
        pdf.cell(w=143, txt='O joelho direito subiu mais %.4s cm' % diff_v)
    else:
        pdf.cell(w=143, txt='O joelho esquerdo subiu mais %.4s cm' % diff_v)

    pdf.set_xy(30, 126)
    r_h, r_v, l_h, l_v, diff_h, diff_v = legs_comparasion(merged, merged_2, 'LM_H', 'LM_V')
    if r_v > l_v:
        pdf.cell(w=143, txt='O tornozelo direito subiu mais %.4s cm' % diff_v)
    else:
        pdf.cell(w=143, txt='O tornozelo esquerdo subiu mais %.4s cm' % diff_v)

    pdf.set_xy(30, 132)
    r_h, r_v, l_h, l_v, diff_h, diff_v = legs_comparasion(merged, merged_2, 'VM_H', 'VM_V')
    if r_v > l_v:
        pdf.cell(w=143, txt='O pé direito subiu mais %.4s cm' % diff_v)
    else:
        pdf.cell(w=143, txt='O pé esquerdo subiu mais %.4s cm' % diff_v)

    pdf.set_xy(30, 140)
    pdf.set_font('times', 'B', 12)
    pdf.cell(w=100, txt='Diferenças nas amplitudes de movimento')
    ...
    if (max(knee_angle) - min(knee_angle)) > (max(knee_angle_2) - min(knee_angle_2)):
        pdf.cell(w=143, txt='O joelho direito teve uma amplitude de movimento de mais %.4sª'
                 % ((max(knee_angle) - min(knee_angle)) - (max(knee_angle_2) - min(knee_angle_2))))
    else:
        pdf.cell(w=143, txt='O joelho esquerdo teve uma amplitude de movimento de mais %.4sª'
                 % (max(knee_angle_2) - min(knee_angle_2)))

    pdf.set_xy(30, 152)
    if max(ankle_angle) > max(ankle_angle_2):
        pdf.cell(w=143, txt='O pé direito teve mais %.4sª de dorsiflexão' % (max(ankle_angle) - max(ankle_angle_2)))
    else:
        pdf.cell(w=143, txt='O pé esquerdo teve mais %.4sª de dorsiflexão' % (max(ankle_angle_2) - max(ankle_angle)))

    pdf.set_xy(30, 158)
    if min(ankle_angle) < min(ankle_angle_2):
        pdf.cell(w=143, txt='O pé direito teve mais %.4sª de flexão plantar' % (abs(min(ankle_angle) - min(ankle_angle_2))))
    else:
        pdf.cell(w=143, txt='O pé esquerdo teve mais %.4sª de flexão plantar' % (abs(min(ankle_angle_2) - min(ankle_angle))))

    pdf.set_xy(30, 166)
    pdf.set_font('times', 'B', 12)
    pdf.cell(w=100, txt='Duração da fase de apoio')
    pdf.set_xy(30, 172)
    pdf.set_font('times', '', 12)

    if r_stance_phase_duration > l_stance_phase_duration:
        pdf.cell(w=143, txt='A fase de apoio do membro direito, %.4ss, foi %.4s%% superior ao do membro esquerdo, %.4ss' % (
        ...
        pdf.cell(w=143, txt='A fase de apoio do membro esquerdo, %.4ss, foi %.4s%% superior ao do membro direito, %.4ss' % (
    ...
    if 'frontal_h' not in globals() and 'frontal_v' not in globals():
        if comments != '':
            pdf.set_xy(30, 185)
            pdf.set_font('times', 'B', 14)
            ...
            pdf.set_xy(30, 192)
            pdf.set_font('times', '', 12)
            pdf.multi_cell(w=143, txt=comments, border=1)
        else:
            pdf.set_xy(30, 185)
            pdf.set_font('times', 'B', 14)
            pdf.cell(w=40, txt='Comentários')
            pdf.set_xy(30, 192)
            pdf.set_font('times', '', 12)
            pdf.multi_cell(w=143, txt='Sem comentários', border=0)

    else:

        pdf.set_xy(30, 185)
        pdf.set_font('times', 'B', 14)
        pdf.cell(w=143, txt='Plano frontal ', align='c')

        pdf.set_font('times', 'B', 12)
        pdf.set_xy(30, 191)
        pdf.cell(w=143, txt='Desvios verticais e médio-laterais')

        pdf.set_font('times', '', 12)
        pdf.set_xy(30, 197)
        if 'LGT_H'not in merged_frontal or 'RGT_H'not in merged_frontal:
            pdf.cell(w=143, txt='O marcaddor do trocânter, LGT/RGT não foi utilizado')
        else:
            L_H, R_H, dif_H, L_V, R_V, dif_V = markers_dif(merged_frontal, 'LGT_H', 'LGT_V', 'RGT_H', 'RGT_V')
            if L_H > R_H:
                pdf.multi_cell(w=143, txt='O marcador LGT movimentou-se mais %.4s cm do que o marcador RGT,'
                         ' na direção horizontal' % dif_H, align='j')
            else:
                pdf.multi_cell(w=143, txt='O marcador RGT movimentou-se mais %.4s cm do que o marcador LGT,'
                         ' na direção horizontal' % dif_H, align='j')

            pdf.set_xy(30, 207)
            if L_V > R_V:
                pdf.cell(w=143, txt='O marcador LGT subiu mais %.4s cm do que o marcador RGT' % dif_V)
            else:
                pdf.cell(w=143, txt='O marcador RGT subiu mais %.4s cm do que o marcador LGT' % dif_V)

        pdf.set_xy(30, 213)
        if 'LLE_H' not in merged_frontal or 'RLE_H' not in merged_frontal:
            pdf.cell(w=143, txt='O marcaddor do epicôndilo lateral, LLE/RLE, não foi utilizado')
        else:
            L_H, R_H, dif_H, L_V, R_V, dif_V = markers_dif(merged_frontal, 'LLE_H', 'LLE_V', 'RLE_H', 'RLE_V')
            if L_H > R_H:
                pdf.cell(w=143, txt='O marcador LLE teve uma rotação externa de maix %.4s cm' % dif_H)
            else:
                pdf.cell(w=143, txt='O marcador RLE teve uma rotação interna de mais %.4s cm' % dif_H)

            pdf.set_xy(30, 219)
            if L_V > R_V:
                pdf.cell(w=143, txt='O marcador LLE subiu mais %.4s cm do que o marcador RLE' % dif_V)
            else:
                pdf.cell(w=143, txt='O marcador RLE subiu mais %.4s cm do que o marcador LLE' % dif_V)

        pdf.set_xy(30, 225)
        if 'LTT_H' not in merged_frontal or 'RTT_H' not in merged_frontal:
            pdf.multi_cell(w=143, txt='O marcaddor da tuberosidade da tibia, LTT/RTT, não foi utilizado')
        else:
            L_H, R_H, dif_H, L_V, R_V, dif_V = markers_dif(merged_frontal, 'LTT_H', 'LTT_V', 'RTT_H', 'RTT_V')

            if L_H > R_H:
                pdf.cell(w=143, txt='O marcador LTT teve uma rotação externa de mais %.4s cm' % dif_H)
            else:
                pdf.cell(w=143, txt='O marcador RTT teve uma rotação interna de mais %.4s cm' % dif_H)

            pdf.set_xy(30, 231)
            if L_V > R_V:
                pdf.cell(w=143, txt='O marcador LTT subiu mais %.4s cm do que o marcador RTT' % dif_V)
            else:
                pdf.cell(w=143, txt='O marcador RTT subiu mais %.4s cm do que o marcador LTT' % dif_V)

        pdf.set_xy(30, 237)
        if 'LLM_H' not in merged_frontal or 'RLM_H' not in merged_frontal:
            pdf.multi_cell(w=143, txt='O marcaddor do maleolo lateral, LLM/RLM, não foi utilizado')
        else:
            L_H, R_H, dif_H, L_V, R_V, dif_V = markers_dif(merged_frontal, 'LLM_H', 'LLM_V', 'RLM_H', 'RLM_V')

            if L_H > R_H:
                pdf.cell(w=143, txt='O marcador LLM teve uma rotação externa de maix %.4s cm' % dif_H)
            else:
                pdf.cell(w=143, txt='O marcador RLM teve uma rotação interna de mais %.4s cm' % dif_H)

            pdf.set_xy(30, 243)
            if L_V > R_V:
                pdf.cell(w=143, txt='O marcador LLM subiu mais %.4s cm do que o marcador RTT' % dif_V)
            else:
                pdf.cell(w=143, txt='O marcador RLM subiu mais %.4s cm do que o marcador LTT' % dif_V)

        pdf.set_xy(30, 249)
        if 'LVM_H'not in merged_frontal or 'RVM_H' not in merged_frontal:
            pass
        else:
            L_H, R_H, dif_H, L_V, R_V, dif_V = markers_dif(merged_frontal, 'LVM_H', 'LVM_V', 'RVM_H', 'RVM_V')

            if L_H > R_H:
                pdf.cell(w=143, txt='O marcador LVM teve uma rotação externa de mais %.4s cm' % dif_H)
            else:
                pdf.cell(w=143, txt='O marcador RVM teve uma rotação externa de mais %.4s cm' % dif_H)

            pdf.set_xy(30, 255)
            if L_V > R_V:
                pdf.cell(w=143, txt='O marcador LVM subiu mais %.4s cm' % dif_V)
            else:
                pdf.cell(w=143, txt='O marcador RVM subiu mais %.4s cm do que o marcador LVM' % dif_V)
    ...
    pdf.set_font('times', 'B', 14)
    pdf.set_xy(30, 70)
    pdf.multi_cell(w=143, txt='Comparação dos movimentos angulares\ndas articulações de cada membro', align='c')
    pdf.set_xy(20, 83)
    ...
    if 'frontal_h' in globals() and 'frontal_v' in globals():
        pdf.add_page()
        pdf.image('logo.png', x=10, y=20, w=65, h=37)
        pdf.set_font('times', 'B', 22)
        pdf.set_xy(65, 30)
        ...
        if comments !='':
            pdf.set_xy(30, 65)
            pdf.set_font('times', 'B', 14)
            pdf.cell(w=100, txt='Comentários')
            pdf.set_font('times', '', 12)
            pdf.set_xy(30, 71)
            pdf.multi_cell(w=143, txt=comments, border=1)
...
def hip_angle():
    ...
    sub_plotting(freq_left, hip_angle_2, 'Ângulo da anca - Membro esquerdo', ax1)
    sub_plotting(freq_right, hip_angle, 'Ângulo da anca - Membro direito', ax2)
...
def ankle_angle():
    ...
    sub_plotting(freq_left, ankle_angle_2, 'Ângulo do tornozel - Membro esquerdo', ax1)
    sub_plotting(freq_right, ankle_angle, 'Ângulo do tornozel - Membro direito', ax2)
    ax1.set_xlabel('Fase da marcha (%)')
    ax2.set_xlabel('Fase da marcha (%)')
    ax1.set_ylabel('Ângulo (graus)')
    ax2.set_ylabel('Ângulo (graus)')
...
def sides_comparison():
    ...
    if (max(knee_angle) - min(knee_angle)) > (max(knee_angle_2) - min(knee_angle_2)):
        st.write('O joelho direito teve uma amplitude de movimento de mais %.4sª'
                 % ((max(knee_angle) - min(knee_angle)) - (max(knee_angle_2) - min(knee_angle_2))))
    else:
        st.write('O joelho esquerdo teve uma amplitude de movimento de mais %.4sª'
                 % (max(knee_angle_2) - min(knee_angle_2)))
    ...
    if r_stance_phase_duration > l_stance_phase_duration:
        st.write('A fase de apoio da membro direito, %.4ss, foi %.4s%% superior ao do membro esquerdo, %.4ss' % (
                r_stance_phase_duration, (r_stance_phase_duration - l_stance_phase_duration) / r_stance_phase_duration * 100,
                l_stance_phase_duration))
    else:
        st.write('A fase de apoio da membro esquerdo, %.4ss, foi %.4s%% superior ao d membro direito, %.4ss' % (
                l_stance_phase_duration, (l_stance_phase_duration - r_stance_phase_duration) / l_stance_phase_duration * 100,
                r_stance_phase_duration))
...
with st.sidebar:
    distance = st.number_input('Distância ao trocânter (cm)', step=0.1,
...
with st.sidebar:
    with st.expander('Definições avançadas'):
        fps_rate = st.number_input('Taxa de FPS', value=120, step=10,
                                   help='Taxa de FPS (Frames per second) usada durante a recolha de imegens')

        min_lim_right_leg = st.number_input('Início da fase de balanço (%) - Membro direito', value=55, step=1,
                                  help='Valor mínimo do intervalo onde ocorre a transição da fase de apoio'
                                       ' para a fase de balanço')

        min_lim_left_leg = st.number_input('Início da fase de balanço (%) - Membro esquerdo', value=55, step=1,
                                            help='Valor mínimo do intervalo onde ocorre a transição da fase de apoio'
                                                 ' para a fase de balanço')
...
if plane == 'Sagital':
    ...
    with st.container():
        ...
        comp_ref = st.checkbox('Comparar com valores de referência')
        st.write('')
    ...
    with st.container():
        if path:
            ...
            if option == 'Joelho':
                with st.spinner('A processar os dados...'):
                    ...
                    if detect_marker('GT_H', merged) == False or detect_marker('LE_H', merged) == False or \
                        ...
                        sub_plotting(freq_left, knee_angle_2 - knee_angle_2[0], 'Ângulo do joelho - Membro esquerdo', ax1)
                        sub_plotting(freq_right, knee_angle - knee_angle[0], 'Ângulo do joelho - Membro direito', ax2)
                        ax1.set_xlabel('Fase da marcha (%)')
                        ax2.set_xlabel('Fase da marcha (%)')
                        ax1.set_ylabel('Ângulo (graus)')
                        ax2.set_ylabel('Ângulo (graus)')
            ...
            if report_checkbox:
                ...
                if amputation_level == 'Transfemoral':
                    knee_ref = st.text_input('Referência do joelho', value='')
                foot_ref = st.text_input('Referência do pé', value='')
                susp_type = st.text_input('Tipo de suspensão', value='')
    ...
    with st.container():
        ...
        if comp_ref:
            if files == []:
                ...
                col1, col2 = st.columns([1.5, 1])
                with col1:
                    option_comp = st.selectbox('Escolha o membro que pretende analisar',
                                               {'Membro amputado', 'Membro contralateral'}, index=0)

                col1, col2 = st.columns(2)
                if option_comp == 'Membro amputado':
                    if amputation_level == 'Transtibial' and amputated_leg == 'Direito':
                        with col1:
                            st.write('**Resultados**')
                    ...
                    if amputation_level == 'Transtibial' and amputated_leg == 'Esquerdo':
                        with col1:
                            st.write('**Resultados**')
                    ...
                    if amputation_level == 'Transfemoral' and amputated_leg == 'Direito':
                        with col1:
                            st.write('**Resultados**')
                            img_comparison(merged, freq_right)
                    ...
                    if amputation_level == 'Transfemoral' and amputated_leg == 'Esquerdo':
                        with col1:
                            st.write('**Resultados**')
                            img_comparison(merged_2, freq_left)
                ...
                if option_comp == 'Membro contralateral':
                    if amputated_leg == 'Direito':
                        with col1:
                            st.write('**Resultados**')
                            img_comparison(merged_2, freq_left)
